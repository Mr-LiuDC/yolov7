...
names = ["box", "cup", "bowl", "plate", "independent"]
...
class ONNX_engine():
    ...
    def predict(self, im):
        output_name = [i.name for i in self.session.get_outputs()]
        input_name = [i.name for i in self.session.get_inputs()]
        inp = {input_name[0]: im}
        outputs = self.session.run(output_name, inp)[0]
        # print(outputs.shape)
        return outputs
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='./weights/5-classes-food-container.onnx',
                        help='weights path of onnx')
